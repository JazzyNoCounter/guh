...
ctypes.windll.kernel32.SetConsoleTitleW(f"Boosts Bot V.2 | SEA#0001 ")
...
@bot.event
async def on_ready():
    activity = discord.Game(name="paypal.me/SeaOnShop", type=2)
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="paypal.me/SeaOnShop"))
    print(f"""
██████╗ ██╗   ██╗███████╗██╗  ██╗
██╔══██╗╚██╗ ██╔╝██╔════╝╚██╗██╔╝
██████╔╝ ╚████╔╝ █████╗   ╚███╔╝ 
...
async def boost(ctx, inv, amount: int):
    ...
    embed.set_footer(text = f"paypal.me/SeaOnShop | SEA#0001", icon_url = "")
